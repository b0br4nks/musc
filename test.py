...
def test(folder: str):
    ...
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and entry.path.endswith(skorpio_ext):
            ...
            sim_cmd = cmd_run_echoed(["./skorpio.py", "-s", entry.path], capture_output=True)
            ...
            com_cmd = cmd_run_echoed(["./skorpio.py", "-c", "-r", "--silent", entry.path], capture_output=True)
...
def record(folder: str, mode: str='-s'):
    for entry in os.scandir(folder):
        ...
        if entry.is_file() and entry.path.endswith(skorpio_ext):
            if mode == '-s':
                output = cmd_run_echoed(["./skorpio.py", "-s", entry.path], capture_output=True)
            elif mode == '-c':
                output = cmd_run_echoed(["./skorpio.py", "-c", "-r", "-s", entry.path], capture_output=True)
            else:
