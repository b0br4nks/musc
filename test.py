...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    if tc is not None:
        sim = cmd_run_echoed([sys.executable, "./skorpio.py", "-s", file_path, *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed([sys.executable, "./skorpio.py", "-c", "-r", "--silent", file_path, *tc.argv], input=tc.stdin, capture_output=True)
        ...
        com = cmd_run_echoed([sys.executable, "./skorpio.py", "-c", file_path])
        if com.returncode != 0:
            stats.com_failed += 1
...
def run_test_for_folder(folder: str):
    ...
    print()
    print("     Simulation failed: %d, Compilation failed: %d, Ignored: %d\n" % (stats.sim_failed, stats.com_failed, stats.ignored))
...
def update_output_for_file(file_path: str):
    ...
    output = cmd_run_echoed([sys.executable, "./skorpio.py", "-s", file_path, *tc.argv], input=tc.stdin, capture_output=True)
